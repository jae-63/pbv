...
var CachePad = class {
  ...
  getChildren() {
    return this.items.map(
      (sym, i2) => new CachePadItem(i2 + 1, sym, this.recentSet.has(sym))
    );
  }
  // --- Public API -------------------------------------------------------
  getItems() {
  ...
  prepend(symbol) {
    ...
    this.items = [symbol, ...this.items.filter((s2) => s2 !== symbol)].slice(0, this.maxItems);
  ...
  absorbEdit(event) {
    for (const change of event.contentChanges) {
      ...
      while ((m2 = IDENTIFIER_RE.exec(text)) !== null) {
        this.prepend(m2[0]);
      }
    }
  }
  ...
  absorbDocument(doc) {
    ...
    IDENTIFIER_RE.lastIndex = 0;
    while ((m2 = IDENTIFIER_RE.exec(text)) !== null) {
      if (!STOP_WORDS.has(m2[0])) found.push(m2[0]);
    }
...
function resolveModLine(targetMod, editor) {
  ...
  for (let l2 = 0; l2 < count; l2++) {
    if ((l2 + 1) % 100 === mod) {
      const dist = Math.abs(l2 - current);
      if (dist < bestDist) {
        bestDist = dist;
        bestLine = l2;
      }
...
var IpcServer = class {
  ...
  broadcast(msg) {
    ...
    for (const s2 of this.sockets) {
      if (!s2.destroyed) s2.write(line);
    }
  }
  dispose() {
    for (const s2 of this.sockets) s2.destroy();
    this.server.close();
  }
  // vscode.Disposable
  [Symbol.dispose]() {
...
var BetaMessageStream = class _BetaMessageStream {
  ...
  [(_BetaMessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_listeners = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_ended = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_errored = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_aborted = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_response = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_request_id = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_handleError = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_instances = /* @__PURE__ */ new WeakSet(), _BetaMessageStream_getFinalMessage = function _BetaMessageStream_getFinalMessage2() {
    ...
    if (this.ended)
      return;
    __classPrivateFieldSet6(this, _BetaMessageStream_currentMessageSnapshot, void 0, "f");
...
var MessageStream = class _MessageStream {
  ...
  [(_MessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _MessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_listeners = /* @__PURE__ */ new WeakMap(), _MessageStream_ended = /* @__PURE__ */ new WeakMap(), _MessageStream_errored = /* @__PURE__ */ new WeakMap(), _MessageStream_aborted = /* @__PURE__ */ new WeakMap(), _MessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _MessageStream_response = /* @__PURE__ */ new WeakMap(), _MessageStream_request_id = /* @__PURE__ */ new WeakMap(), _MessageStream_handleError = /* @__PURE__ */ new WeakMap(), _MessageStream_instances = /* @__PURE__ */ new WeakSet(), _MessageStream_getFinalMessage = function _MessageStream_getFinalMessage2() {
    ...
    if (this.ended)
      return;
    __classPrivateFieldSet7(this, _MessageStream_currentMessageSnapshot, void 0, "f");
...
var SYSTEM_PROMPT = `You are a voice coding assistant embedded in VSCode. The user speaks and you interpret their utterance as a single editor command, returning a JSON object.

AVAILABLE COMMANDS (return exactly one):

Insertion/editing:
  {"cmd":"insertText","text":"..."}           insert at cursor; \\n for newlines
  {"cmd":"replaceSelection","text":"..."}     replace selected text (use for code transformations)

Navigation:
  {"cmd":"gotoLine","line":N}
  {"cmd":"gotoWordOnLine","line":N,"word":N}  word is 1-based
  {"cmd":"cursorUp","n":N}
  {"cmd":"cursorDown","n":N}
  {"cmd":"cursorLeft"}
  {"cmd":"cursorRight"}
  ...
  {"cmd":"pageUp"}
  {"cmd":"pageDown"}
  {"cmd":"selectToken","token":"..."}         find and select an identifier

Cache pad (1-based index into the list provided):
  {"cmd":"insertCacheItem","index":N}
...
RULES:
- Respond with ONLY the JSON object. No prose, no markdown fences, nothing else.
- For dictation (code or text to type at cursor): insertText.
- For transforming selected code ("make this async", "add error handling", "convert to list comprehension"): replaceSelection with the transformed code.
- Line and word numbers are 1-based.
- Prefer a named command over insertText when the utterance clearly describes an action.
- Use cache pad items to resolve ambiguous identifier names.
- For insertText, format code correctly for the language (match indentation style of the surrounding context).`;
var ClaudeClient = class {
  constructor(apiKey) {
    this.client = new sdk_default({ apiKey });
  }
  async interpret(transcript, snap) {
    const contextLines = windowAroundCursor(snap.content, snap.cursorLine, 100);
    const userMsg = [
      `File: ${snap.fileName} (${snap.language})`,
    ...
    try {
      ...
      const raw = response.content.filter((b2) => b2.type === "text").map((b2) => b2.text).join("").trim();
      return JSON.parse(raw);
    } catch (err) {
      vscode5.window.showWarningMessage(`Voice Coder: Claude error \u2014 ${err}`);
      return null;
    }
  }
};
function windowAroundCursor(content, cursorLine, radius) {
  const lines = content.split("\n");
  const start = Math.max(0, cursorLine - 1 - radius);
  const end = Math.min(lines.length, cursorLine + radius);
  return lines.slice(start, end).map((l2, i2) => `${start + i2 + 1}: ${l2}`).join("\n");
}
...
function activate(context) {
  ...
  const claude = apiKey ? new ClaudeClient(apiKey) : void 0;
